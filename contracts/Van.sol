...
contract VansCollection is ERC721A, ERC2981, Ownable {
    ...
    IERC721Enumerable constant DIPPIES_CONTRACT =
        IERC721Enumerable(0xaE7F5a4733Aa13cE0F983Dcb318c52C288BfB1b4);
    ...
    function mint(
        ...
        require(
            state == Status.LIVE || state == Status.PRESALE,
            "Vans: Mint Not Active"
        );
        require(
            msg.sender == tx.origin,
            "Vans: Contract Interaction Not Allowed"
        );
        require(
            ECDSA.recover(
        ...
        require(
            totalSupply() + quantity <= MINT_SUPPLY,
            "Vans: Mint Supply Exceeded"
        );
        require(
            msg.value >=
                (quantity *
                    (state == Status.LIVE ? MINT_PRICE : ALLOW_LIST_PRICE)),
            "Vans: Insufficient ETH"
        );

        if (state == Status.PRESALE) {
            require(
                _numberMinted(msg.sender) + quantity <= MAX_PER_ALLOW_LIST,
                "Vans: Exceeds Max Per Wallet"
            );
        }
    ...
    function claim() external {
        ...
        for (uint256 i; i < DIPPIES_CONTRACT.balanceOf(msg.sender); i++) {
            uint256 tokenId = DIPPIES_CONTRACT.tokenOfOwnerByIndex(
                msg.sender,
                i
            );
    ...
    function dippieBalanceView(address _address)
        external
        view
        returns (uint256)
    {
    ...
    function setBaseURI(string memory _newBaseURI, bool _revealed)
        external
        onlyOwner
    {
        baseURI = _newBaseURI;
    ...
    function sendRemainingToTreasury(uint256 _qty) external onlyOwner {
        require(
            state == Status.ENDED,
            "Vans: Cannot Claim Unminted Tokens If Sale Live"
        );
        require(
            totalSupply() + _qty <= MAX_SUPPLY,
            "Vans: Total Supply Minted"
        );
    ...
    function reserveTeamTokens(uint256 quantity) external onlyOwner {
        require(
            _reservedTeamTokens + quantity <= 150,
            "Dippies: Team Tokens Already Minted"
        );
    ...
    function withdraw() external onlyOwner {
        ...
        payable(0x3428baB379a1D3091653938338d4BBB8b4B07547).transfer(
            (balance * 3) / 10
        );
        payable(0x64BC86B593C9F0F371D98660F9579507F442e6C9).transfer(
            (balance * 3) / 10
        );
        payable(0xC1433f0D731a30F5DCbBDd3Bbb8E72b538FFb9bC).transfer(
            (balance * 39) / 200
        );
        payable(0x2795033F658E8E2a8DeB1A28C7c65d743aA3154C).transfer(
            (balance * 3) / 40
        );
        payable(0x249C406f08C79c3220c3D401B77e7155fD706021).transfer(
            (balance * 1) / 20
        );
        payable(0xaAA217e121433F6482f78371910930FC1AcA226b).transfer(
            (balance * 1) / 20
        );
        payable(0xEc53FDe80883708751B57DeC6914B12F12147a3b).transfer(
